...
pub enum Error {
  ...
  /// errors.
  #[error("RLP error")]
  LegacyRlp(#[from] rlp::DecoderError),
}
...
use reth_proofs_core::mpt::MptNodeReference;
      ...
      rlp::Prototype::List(17) => {
        ...
        for node_rlp in rlp.iter().take(16) {
          match node_rlp.prototype()? {
            ...
            _ => node_list.push(Some(Box::new(rlp::Decodable::decode(&node_rlp)?))),
          }
        }
...
impl MptNode {
        ...
        if remaining.next().is_none() {
          ...
          match &mut orphan.data {
            ...
            MptNodeData::Leaf(prefix, orphan_value) => {
              let new_nibs: Vec<_> = core::iter::once(index as u8)
  ...
  pub fn debug_rlp<T: alloy_rlp::Decodable + core::fmt::Debug>(&self) -> Vec<String> {
...
pub fn resolve_nodes(
  ...
  let trie = match root.as_data() {
    ...
    MptNodeData::Extension(prefix, target) => {
      MptNodeData::Extension(prefix.clone(), Box::new(resolve_nodes(target, node_store))).into()
    }
    MptNodeData::Digest(digest) => {
